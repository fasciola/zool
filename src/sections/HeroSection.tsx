...
export function HeroSection() {
  ...
  // Video IntersectionObserver: pause when offscreen
  useEffect(() => {
  ...
  // GSAP Animations: Parallax and Clip Reveal
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 2. Parallax scroll of video background (fades & moves)
  ...
  return (
    ...
    >
      {/* 1. Background Video with deep local solid fallbacks */}
      <div className="absolute inset-0 w-full h-full overflow-hidden select-none z-0">
        ...
        {/* Professional Polish Ambient Blue Background Blurs */}
        <div className="absolute top-0 right-0 w-full h-full opacity-15 pointer-events-none">
          <div className="absolute top-[-20%] right-[-20%] w-[80%] h-[80%] rounded-full bg-[#2B64F6] blur-[150px]" />
          <div className="absolute bottom-[-20%] left-[-20%] w-[60%] h-[60%] rounded-full bg-[#528BFF] blur-[120px] opacity-40" />
        </div>
      ...
      >
        ...
        <div className="hero-fade-in inline-flex items-center gap-2 px-4 py-2 rounded-sm bg-[#2B64F6]/10 border border-[#2B64F6] text-[#2B64F6] text-[11px] sm:text-xs font-mono font-bold uppercase tracking-widest mb-6 shadow-sm">
        ...
        <div className="hero-fade-in flex flex-col sm:flex-row items-center justify-center gap-4 mt-10 w-full sm:w-auto font-mono">
          <button
            ...
            className="flex w-full sm:w-auto justify-center items-center gap-2.5 px-8 py-4 bg-[#2B64F6] hover:bg-[#528BFF] active:scale-95 text-white font-bold font-mono tracking-wider rounded-sm shadow-xl transition-all duration-300 uppercase cursor-pointer text-sm"
          >
